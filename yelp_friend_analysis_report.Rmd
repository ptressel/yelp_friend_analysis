...
cmd = "INSERT IGNORE INTO %s (val) VALUE (%s);"
try:
    cursor.execute(cmd % ("after", 0))
    cursor.execute(cmd % ("after", 1))
    cursor.execute(cmd % ("friend", 0))
    cursor.execute(cmd % ("friend", 1))
    cnx.commit()
except mysql.connector.Error as err:
...
# Repeat the step of joining the four cases for the pre and post site
...
# Check if the tables are already populated, and quit if so.
# Check just the main result table.
if not is_table_empty("friend_review_statistics_post_all_cases"):
    sys.exit(0)

case_table = "friend_review_statistics_%(version)s_%(friend)s_%(after)s"

insert_case = """
INSERT INTO friend_review_statistics_%(version)s_%(friend)s_%(after)s
SELECT business_key, user_key,
stars,
review_count,
avg_review_stars,
std_review_stars
FROM friend_review_statistics_%(version)s
WHERE is_friend = %(friend)s AND is_after = %(after)s
"""

join_table = "friend_review_statistics_%(version)s_all_cases"
join_cases = """
INSERT INTO friend_review_statistics_%(version)s_all_cases
SELECT
s00.business_key,
s00.user_key,
s00.stars AS reference_stars,
...
s11.std_review_stars AS std_review_stars_1_1
FROM
friend_review_statistics_%(version)s_0_0 as s00,
friend_review_statistics_%(version)s_0_1 as s01,
friend_review_statistics_%(version)s_1_0 as s10,
friend_review_statistics_%(version)s_1_1 as s11
WHERE
s00.business_key = s01.business_key
AND s00.business_key = s10.business_key
AND s00.business_key = s11.business_key
...
for version in ("pre", "post"):
    ...
    for friend in (0, 1):
        for after in (0, 1):
            case = {"version": version, "friend": friend, "after": after}
            populate_table(cnx, cursor, case_table % case, insert_case % case)
 
    # Join the case tables.
    name = {"version": version}
    populate_table(cnx, cursor, join_table % name, join_cases % name)

cnx.close()
```

```{python extract_friend_statistics, echo=FALSE}

# Extract the data for the friend investigation, so it can be loaded into R.
# The business and user keys are no longer relevant -- they were only required
# to perform the aggregation.  Likewise the date is not needed, since we've
# grouped by before and after.

import os.path
import sys
import time
# See comment before commented-out code re. why we can't have MySQL write out
# the file directly.
# from sql_utilities import connect_as_db_user, do_cmd
from sql_utilities import connect_as_db_user, do_select

data_dir = "data"
friend_review_statistics = "friend_review_statistics_rc_ge_3.csv"
csv_file = os.path.join(data_dir, friend_review_statistics)
...
# So, instead, read the data through Python.  This is a wasted hop, but is
# safer and less worry for anyone running this, as this process will run as
# the user's own account, and have their own file permissions.
csv_header = [
"reference_stars", 
"review_count_0_0", "avg_review_stars_0_0", "std_review_stars_0_0",
"review_count_0_1", "avg_review_stars_0_1", "std_review_stars_0_1",
"review_count_1_0", "avg_review_stars_1_0", "std_review_stars_1_0",
"review_count_1_1", "avg_review_stars_1_1", "std_review_stars_1_1"
]
cmd = r"""
SELECT
reference_stars, 
...
review_count_1_0, avg_review_stars_1_0, std_review_stars_1_0,
review_count_1_1, avg_review_stars_1_1, std_review_stars_1_1
FROM friend_review_statistics_all_cases
WHERE review_count_0_0 >= 3
AND review_count_0_1 >= 3
AND review_count_1_0 >= 3
AND review_count_1_1 >= 3;
"""
do_select(cnx, cursor, cmd, csv_file, csv_header)

cnx.close()
```

```{python extract_friend_statistics_split_dates, echo=FALSE}

# Repeat extracting the friend statistics for the pre and post site
# reorganization data.
...
data_dir = "data"
friend_review_statistics_split = "friend_review_statistics_%s_rc_ge_3.csv"
csv_file = os.path.join(data_dir, friend_review_statistics_split)

# Check whether we have already created the files produced by this chunk,
# and exit if so. Check only one of the files.
if os.path.exists(csv_file % "post"):
    sys.exit(0)
...
review_count_1_0, avg_review_stars_1_0, std_review_stars_1_0,
review_count_1_1, avg_review_stars_1_1, std_review_stars_1_1
FROM friend_review_statistics_%s_all_cases
WHERE review_count_0_0 >= 3
AND review_count_0_1 >= 3
AND review_count_1_0 >= 3
AND review_count_1_1 >= 3;
"""
do_select(cnx, cursor, cmd % "pre", csv_file % "pre", csv_header)
do_select(cnx, cursor, cmd % "post", csv_file % "post", csv_header)
